/** Content for the "👤 About Dev" popup. */
export function AboutDevContent() {
  return (
    <div className="rules-content">
      <p>Hey, I'm chinkyyydev 👋</p>
      <p>
        I work a regular 9-to-5, and PokéBrawl is the first time I've ever <strong>"vibe
        coded"</strong> a full app — built nights and weekends, mostly by describing what I
        wanted and iterating with AI rather than writing every line by hand.
      </p>
      <p>
        It's still very much a work in progress (and devnet-only for now!), so thanks for trying
        it out. Bug reports and feedback are always welcome.
      </p>
      <p>
        <a href="https://github.com/chinkyyydev/pokebrawl" target="_blank" rel="noopener noreferrer">
          🔗 github.com/chinkyyydev/pokebrawl
        </a>
      </p>
    </div>
  );
}
